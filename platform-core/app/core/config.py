...
class Settings(BaseModel):
    ...
    jwt_secret_key: str = os.getenv("JWT_SECRET_KEY", "replace-with-a-long-random-local-secret")
    ...
    database_url: str = os.getenv(
        "DATABASE_URL",
        "postgresql+psycopg://app_user:change-me-local-dev@localhost:5432/app_db",
    )
    redis_url: str = os.getenv("REDIS_URL", "redis://localhost:6379/0")
    bootstrap_sample_data: bool = os.getenv("BOOTSTRAP_SAMPLE_DATA", "false").lower() == "true"
    bootstrap_auth_users: bool = os.getenv("BOOTSTRAP_AUTH_USERS", "false").lower() == "true"
